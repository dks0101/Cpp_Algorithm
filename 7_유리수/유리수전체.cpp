...
public:
    ...
    long long _den;
    // Constructor
    myRational(long long num=0, long long den=1) {
        if (den == 0) {
            num = 0;
            den = 1;
        }
    ...
    // Accessor
    long long getNumerator() const { return _num; }
    long long getDenominator() const { return _den; }
    // Member Function
    ...
    void reduce() {
        ...
        long long divisor = gcd(_num, _den);
        _num /= divisor; _den /= divisor;
        if (negative) _num = -_num;
    }
    // Overloaded Operator
    myRational operator +(const myRational &rat) {
        long long den = lcm(_den, rat._den);
        long long num = _num * (den / _den) + rat._num * (den / rat._den);
